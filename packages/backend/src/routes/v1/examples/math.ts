...
const mathRoutes: FastifyPluginAsync = async function (fastify, opts) {
  ...
  fastify.get<{ Params: { a: string; b: string } }>('/mult/:a/:b', async function (request, reply) {
    const { a, b } = request.params;
    return { result: multiply(Number(a), Number(b)) };
  });

  fastify.get<{ Querystring: { a: string; b: string } }>('/div', async function (request, reply) {
    const { a, b } = request.query;
